...
namespace Valve.VR.InteractionSystem
{
    ...
    public class Interactable : MonoBehaviour
    {
        ...
        protected virtual void OnDestroy()
        {
            ...
            {
                // Hand will detect null object on next update, no need to detach explicitly during destruction
                // which causes "Cannot set parent" errors.
                /*
                Hand hand = attachedToHand;
                try
                {
                    hand.DetachObject(this.gameObject, false);
                }
                catch (System.Exception)
                {
                    // Ignore errors if object is already being destroyed
                }

                if (hand != null && hand.skeleton != null)
                {
